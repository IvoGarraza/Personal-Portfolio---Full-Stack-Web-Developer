...
const Hero = () => {
  ...
  return (
    <div className='flex flex-col items-center justify-center bg-[#1B262C] h-[90vh]'>
      <div className='flex flex-row items-center justify-around w-[100%]'>
        <div className='ml-[5%] text-black '>
          ...
          <br />
          <br />
          <motion.span initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 1, duration: 0.5 }} className='text-white text-3xl' >Soy </motion.span>
          {name.map((letter, index) => (
            <TextSpan key={index} size='text-3xl' weight='font-bold' color='text-white'>
              {letter === " " ? "\u00A0" : letter}
            </TextSpan>
          ))}
          <br />
        ...
        <motion.div className='' initial={{ y: 100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} transition={{ duration: 1 }}>
      ...
      <motion.div className='absolute mt-[500px] mr-[-1400px]' initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }}>
        ...
        <div class="relative flex items-center group">
          <motion.svg class="w-10 h-10 fill-white animate-pulse cursor-pointer" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" whileTap={() => setOpen(!isOpen)}>
            <path fill-rule="evenodd" d="M18 10a8 8 0 11-16 0 8 8 0 0116 0zm-8-3a1 1 0 00-.867.5 1 1 0 11-1.731-1A3 3 0 0113 8a3.001 3.001 0 01-2 2.83V11a1 1 0 11-2 0v-1a1 1 0 011-1 1 1 0 100-2zm0 8a1 1 0 100-2 1 1 0 000 2z" clip-rule="evenodd" />
          </motion.svg>
          <AnimatePresence>
            {isOpen && (
/*               <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1 }} exit={{opacity:0}} class="absolute top-0 flex flex-col items-center hidden mt-6 group-hover:flex">
              ...
              <motion.div className='w-[200px] h-[100px] bg-slate-700 absolute ml-[-200px] flex items-center text-center rounded-xl' initial={{opacity:0}} animate={{opacity:1}} transition={{duration:0.1}} exit={{opacity:0}}>
                <button className='text-white font-bold absolute mt-[-40%] ml-[90%]' onClick={() => setOpen(prevValue => !prevValue)}>x</button>
                <span className='text-white m-2'>Retrato hecho con <a className='font-bold' href='https://stablediffusionweb.com/'>Stable Diffusion</a></span>
