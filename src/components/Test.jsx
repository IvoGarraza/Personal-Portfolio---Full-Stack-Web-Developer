...
const Card = (props) => {
  ...
  return (
    ...
    <motion.div onHoverStart={() => setOpen(true)} onHoverEnd={() => setOpen(false)} className={`${props.id} bg-center bg-no-repeat bg-cover bg-start m-2 flex flex-col items-center justify-center rounded-xl cursor-pointer h-[450px] w-[350px] `}>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div className="absolute p-4 h-[450px] w-[350px] flex flex-col items-center justify-center text-white bg-port__gray  rounded-xl font-ubuntu"
            initial={{ scale: 0, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ duration: 0.2 }} exit={{ opacity: 0, scale: 0 }}>
            <p>
              {props.description}
            </p>
            {props.link ? <a href={props.link} className='bg-purple-500 p-2 mt-5 w-[30%] rounded-lg text-white hover:bg-slate-300 hover:text-black bg-purple-500'>Link</a> : <Link to={props.path}><button className="bg-purple-500 p-2 mt-5 rounded-lg text-white hover:bg-slate-300 hover:text-black">Ver mas</button></Link>}
